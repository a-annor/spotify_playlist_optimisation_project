...
def music_period(music_history, start_date, end_date):
    start_date = datetime.datetime.strptime(start_date,'%Y-%m-%d').date()
    end_date =  datetime.datetime.strptime(end_date,'%Y-%m-%d').date()
    mask = (music_history['ts'] >= start_date) & (music_history['ts'] <= end_date)
    time_period = music_history[mask]
    return time_period
...
def rewind_time(my_spotify_history, start_date, end_date):
    ...
    final_playlist=ordered_playlist(playlist_period_features,weighted_tracks)
    
    spotify_new_playlist = sp.user_playlist_create(user = 'aannor', name = start_date +'_' + end_date + '_top_songs_api', public=True, collaborative=False, description='')
    return sp.user_playlist_add_tracks(username, spotify_new_playlist['id'], final_playlist['track_id'])

rewind_time(spotify_history,'2016-01-01','2016-06-30')
